...
from .identifiers import build_pk_name, build_fk_name
...
class ConstraintRule:
    """Base class for a single structural constraint rule."""
    def check(self, tables: Sequence[Table]) -> None:  # pragma: no cover - interface
...
class PrimaryKeyNonEmptyRule(ConstraintRule):
...
class PrimaryKeyNoDuplicatesRule(ConstraintRule):
    def check(self, tables: Sequence[Table]) -> None:
        for t in tables:
...
class PrimaryKeyColumnsExistRule(ConstraintRule):
    def check(self, tables: Sequence[Table]) -> None:
        for t in tables:
            if not t.primary_key:
                continue
...
class PrimaryKeyColumnsNotNullableRule(ConstraintRule):
    def check(self, tables: Sequence[Table]) -> None:
        for t in tables:
            if not t.primary_key:
                continue
...
class PrimaryKeyNoComplexTypesRule(ConstraintRule):
    def check(self, tables: Sequence[Table]) -> None:
        for t in tables:
            if not t.primary_key:
                continue
...
class ForeignKeyTargetTableExistsRule(ConstraintRule):
    def check(self, tables: Sequence[Table]) -> None:
...
class ForeignKeyTargetHasPrimaryKeyRule(ConstraintRule):
    def check(self, tables: Sequence[Table]) -> None:
        idx = build_table_index(tables)
        for src in tables:
            for fk in src.foreign_keys:
...
class ForeignKeyTargetColumnsMatchPrimaryKeyRule(ConstraintRule):
    def check(self, tables: Sequence[Table]) -> None:
        idx = build_table_index(tables)
        for src in tables:
            for fk in src.foreign_keys:
...
class ForeignKeyArityMatchesRule(ConstraintRule):
    def check(self, tables: Sequence[Table]) -> None:
        for src in tables:
            for fk in src.foreign_keys:
...
class ForeignKeySourceColumnsExistRule(ConstraintRule):
    def check(self, tables: Sequence[Table]) -> None:
        for src in tables:
...
class ForeignKeyTargetColumnsExistRule(ConstraintRule):
    def check(self, tables: Sequence[Table]) -> None:
        idx = build_table_index(tables)
        for src in tables:
            for fk in src.foreign_keys:
...
class ForeignKeySourceColumnsNotNullableRule(ConstraintRule):
    def check(self, tables: Sequence[Table]) -> None:
        for src in tables:
            cols = column_index(src)
            for fk in src.foreign_keys:
...
class ForeignKeyTypesMatchExactlyRule(ConstraintRule):
    def check(self, tables: Sequence[Table]) -> None:
        idx = build_table_index(tables)
        for src in tables:
...
class ConstraintNameCollisionRule(ConstraintRule):
    def check(self, tables: Sequence[Table]) -> None:
