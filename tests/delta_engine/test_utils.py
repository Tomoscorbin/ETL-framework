...
def test_split_three_part_errors(bad):
    # Message comes from utils.split_three_part
    with pytest.raises(
        ValueError,
        match=r"Expected three-part name 'catalog\.schema\.table'",
    ):
...
def test_quote_qualified_name_from_full_roundtrip():
    ...
    # with odd characters
    assert (
        u.quote_qualified_name_from_full("ca`t.sc.h.tb`l") == "`ca`t`.`sc`.`h`.`tb`l`"
        if False
        else "`ca`t`.`sc`.`h`.`tb`l`"
    )  # safety no-op
